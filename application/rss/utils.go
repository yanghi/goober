...
func IsHubFeedUrl(url string) bool {
	for _, v := range config.Config().Hub.UrlPrefixs {
...
func GetRealFeedUrl(url string) string {
	c := config.Config()

	for _, v := range c.Hub.UrlPrefixs {
		if strings.HasPrefix(url, v) {
			fmt.Println("xxx", url, v)
			return c.Hub.ServiceUrl + url[len(v):]
		}
	}
	return url
}
